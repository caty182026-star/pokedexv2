...
function render() {
  ...
  const visible = pokemon.filter(mon => (!query || mon.name.includes(query) || String(mon.id).padStart(3,'0').includes(query)) && (activeType === 'all' || mon.types.includes(activeType)) && (region === 'all' || mon.region === region) && (!favoritesOnly || favorites.includes(mon.id))).sort((first, second) => sort === 'name' ? first.name.localeCompare(second.name) : sort === 'weight' ? second.numericWeight - first.numericWeight : sort === 'height' ? second.numericHeight - first.numericHeight : first.id - second.id);
  document.querySelector('#resultsTitle').textContent = favoritesOnly ? 'Your favorites' : region !== 'all' ? `${region} Pokédex` : activeType === 'all' ? 'All Pokémon' : `${activeType} types`;
...
pokemon = fallbackPokemon.map(makePokemon); renderFilters(); render();
