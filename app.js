...
const evolutionFamilies = [
  ['Bulbasaur → Ivysaur → Venusaur',[1,2,3]], ['Charmander → Charmeleon → Charizard',[4,5,6]], ['Squirtle → Wartortle → Blastoise',[7,8,9]], ['Caterpie → Metapod → Butterfree',[10,11,12]], ['Weedle → Kakuna → Beedrill',[13,14,15]], ['Pidgey → Pidgeotto → Pidgeot',[16,17,18]], ['Rattata → Raticate',[19,20]], ['Pichu → Pikachu → Raichu',[172,25,26]], ['Cleffa → Clefairy → Clefable',[173,35,36]], ['Igglybuff → Jigglypuff → Wigglytuff',[174,39,40]], ['Zubat → Golbat → Crobat',[41,42,169]], ['Oddish → Gloom → Vileplume / Bellossom',[43,44,45,182]], ['Eevee → Vaporeon / Jolteon / Flareon',[133,134,135,136]], ['Eevee → Espeon / Umbreon',[133,196,197]], ['Eevee → Leafeon / Glaceon',[133,470,471]], ['Eevee → Sylveon',[133,700]], ['Dratini → Dragonair → Dragonite',[147,148,149]], ['Chikorita → Bayleef → Meganium',[152,153,154]], ['Cyndaquil → Quilava → Typhlosion',[155,156,157]], ['Totodile → Croconaw → Feraligatr',[158,159,160]], ['Treecko → Grovyle → Sceptile',[252,253,254]], ['Torchic → Combusken → Blaziken',[255,256,257]], ['Mudkip → Marshtomp → Swampert',[258,259,260]], ['Ralts → Kirlia → Gardevoir / Gallade',[280,281,282,475]], ['Bagon → Shelgon → Salamence',[371,372,373]], ['Beldum → Metang → Metagross',[374,375,376]], ['Turtwig → Grotle → Torterra',[387,388,389]], ['Chimchar → Monferno → Infernape',[390,391,392]], ['Piplup → Prinplup → Empoleon',[393,394,395]], ['Snivy → Servine → Serperior',[495,496,497]], ['Litwick → Lampent → Chandelure',[607,608,609]], ['Deino → Zweilous → Hydreigon',[633,634,635]], ['Froakie → Frogadier → Greninja',[656,657,658]], ['Rowlet → Dartrix → Decidueye',[722,723,724]], ['Litten → Torracat → Incineroar',[725,726,727]], ['Popplio → Brionne → Primarina',[728,729,730]], ['Grookey → Thwackey → Rillaboom',[810,811,812]], ['Scorbunny → Raboot → Cinderace',[813,814,815]], ['Sobble → Drizzile → Inteleon',[816,817,818]], ['Sprigatito → Floragato → Meowscarada',[906,907,908]], ['Fuecoco → Crocalor → Skeledirge',[909,910,911]], ['Quaxly → Quaxwell → Quaquaval',[912,913,914]]
];
const gigantamaxData = [['charizard','Charizard','G-Max Wildfire',6], ['butterfree','Butterfree','G-Max Befuddle',12], ['pikachu','Pikachu','G-Max Volt Crash',25], ['meowth','Meowth','G-Max Gold Rush',52], ['machamp','Machamp','G-Max Chi Strike',68], ['gengar','Gengar','G-Max Terror',94], ['kingler','Kingler','G-Max Foam Burst',99], ['lapras','Lapras','G-Max Resonance',131], ['eevee','Eevee','G-Max Cuddle',133], ['snorlax','Snorlax','G-Max Replenish',143], ['garbodor','Garbodor','G-Max Malodor',569], ['copperajah','Copperajah','G-Max Steelsurge',879]];
...
const evolutionDialog = document.querySelector('#evolutionDialog');
document.querySelector('#evolutionButton').addEventListener('click', () => { document.querySelector('#evolutionList').innerHTML = evolutionFamilies.map(family => `<div class="evolution-card"><div class="evolution-chain">${family[1].map(id => `<img src="${imageFor(id)}" alt="" loading="lazy">`).join('<span>→</span>')}</div><strong>${family[0]}</strong></div>`).join(''); openDialog(evolutionDialog); });
document.querySelector('#closeEvolution').addEventListener('click', () => closeDialog(evolutionDialog));
evolutionDialog.addEventListener('click', event => { if (event.target === event.currentTarget) closeDialog(event.currentTarget); });
const maxDialog = document.querySelector('#maxDialog');
document.querySelector('#maxButton').addEventListener('click', () => { document.querySelector('#maxList').innerHTML = gigantamaxData.map(item => `<div class="evolution-card"><div class="evolution-chain"><img src="${imageFor(item[3])}" alt="${item[1]}" loading="lazy"></div><strong>${item[1]}</strong><span>${item[2]}</span></div>`).join(''); openDialog(maxDialog); });
document.querySelector('#closeMax').addEventListener('click', () => closeDialog(maxDialog));
maxDialog.addEventListener('click', event => { if (event.target === event.currentTarget) closeDialog(event.currentTarget); });
